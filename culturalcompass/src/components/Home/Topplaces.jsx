...
import { projectData } from "@/data/cityData"; // Import city data
...
const Topplaces = () => {
  ...
  const handleImageClick = (cityId, imageId) => {
            ...
            <Slider ref={sliderRef} {...settings}>
              {projectData.map((item) =>
                item.images.map((image) => (
                  <div
                    key={image.id} // Use the image id as key for each slide
                    className="flex flex-col items-center gap-4 cursor-pointer pl-2 pr-2"
                    onClick={() => handleImageClick(item.id, image.id)} // Pass the city id and image id on click
                  >
                    <div className="flex items-center justify-center overflow-hidden rounded-md">
                      <Image
                        src={image.image}
                        alt={image.title} // Alt text passed from Readmore
                        width={500}
                        height={500}
                        className="object-cover w-[1500px] h-[400px]"
                      />
                    </div>
                    <p className="mt-2 lg:text-2xl font-medium md:text-xl text-sm">{image.title}</p>
                  </div>
                ))
              )}
            </Slider>
